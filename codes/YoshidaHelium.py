...
import time
# ............................................................. Lists
QtKey=['name','m','q','x','y','vx','vy']
QtLab=['Name','Mass','Charge','x','y','vx','vy']
nQt=len(QtKey)
...
# ..................................................... Class Particle
class particle:
  def __init__(self,name,mass,charge,x,y,vx,vy):
    self.name=name
    self.m=mass
    self.q=charge
    self.x=x
...
def SetBaryc():
  ...
  for i,p in enumerate(part):
    ...
    p.vx-=cvx
    p.vy-=cvy
    pos[2*i:2*i+2]=[p.x,p.y]
    vel[2*nP+2*i:2*nP+2*i+2]=[p.vx,p.vy]
  SelectPart(0)
# ...................................................... Reinitialize
def reinit():
  global part,pos,r1,r2,state,v1,v2,vel
...
def accel(pos):
  ...
  fy=ff*np.sin(alpha)
  accvect=[0]*len(pos)
  accvect[::2]=fx.sum(axis=1)/mass
  accvect[1::2]=fy.sum(axis=1)/mass
  return np.array(accvect)
# ....................................................... Root Window
root=Tk()
root.title('Classical Helium Atom (Yoshida)')
...
# .................................................. Create Particles
part=[]
part.append(particle('Nucleus',4.0*mp,2.0*q,0.0,0.0,0.0,0.0))
part.append(particle('Electron 1',me,-q,-r1,0.0,0.0,-v1))
part.append(particle('Electron 2',me,-q,r2,0.0,0.0,v2))
nP=len(part)
# ....................... Initial State, Charges, Masses and Friction
pos=np.array([0.0]*2*nP)
vel=np.array([0.0]*2*nP)
...
while RunAll:
  ...
  if RunMotion:
    # ............................................. Yoshida Algorithm
    pos+=Cx1*dt*vel
    vel+=Cv1*dt*accel(pos)
    pos+=Cx2*dt*vel
    vel+=Cv2*dt*accel(pos)
    pos+=Cx3*dt*vel
    vel+=Cv3*dt*accel(pos)
    pos+=Cx4*dt*vel
    for i,p in enumerate(part):
      p.x,p.y=pos[2*i:2*i+2]
      p.vx,p.vy=vel[2*i:2*i+2]
    ...
    try:
      ...
      pos[2*SelP:2*SelP+2]=[part[SelP].x,part[SelP].y]
      vel[2*SelP:2*SelP+2]=[part[SelP].vx,part[SelP].vy]
